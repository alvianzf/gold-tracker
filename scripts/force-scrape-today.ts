...
// Re-implementing scraper logic here to avoid import issues with tsx and path aliases
...
async function scrapeGoldPrices() {
  console.log('Scraping fresh gold prices...');
...
async function main() {
  ...
  console.log(`Target date range: ${todayStart.toISOString()} - ${todayEnd.toISOString()}`);
  ...
  await prisma.$transaction(async (tx) => {
    // 1. Delete existing price history for today
    const deletedPrices = await tx.priceHistory.deleteMany({
      where: {
        date: {
          gte: todayStart,
          lte: todayEnd,
    ...
    console.log(`Deleted ${deletedPrices.count} existing price records.`);
    ...
    console.log(`Inserted ${scrapedPrices.length} new price records.`);

    // 3. Delete existing daily summaries for today
    const deletedSummaries = await tx.dailyPortfolioSummary.deleteMany({
      where: {
        date: {
          gte: todayStart,
          lte: todayEnd,
    ...
    console.log(`Deleted ${deletedSummaries.count} existing summary records.`);

    // 4. Recalculate and save new summaries
    ...
    for (const user of users) {
      ...
      if (holdingsCount > 0) {
        ...
        console.log(`Saved summary for user ${user.email}`);
      }
    }
  });
