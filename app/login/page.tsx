      ...
      <div className="glass bg-slate-900/60 p-12 border-white/5">
        ...
        <form onSubmit={handleLogin} className="space-y-8">
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] pl-1">Identification</label>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-gold hover:bg-gold-strong text-black font-black py-5 rounded-2xl shadow-gold transition-all hover:scale-105 active:scale-95 flex items-center justify-center mt-4 text-xs uppercase tracking-widest"
          >
            {isLoading ? <Loader2 className="w-6 h-6 animate-spin" /> : 'Enter Vault'}
          </button>
          
          <div className="text-center mt-10 text-[10px] text-slate-500 font-bold uppercase tracking-widest">
            New to the tracker?{' '}
            <Link href={`/register?callbackUrl=${encodeURIComponent(callbackUrl)}`} className="text-gold hover:underline underline-offset-4">
              Create Account
            </Link>
          </div>
        </form>
